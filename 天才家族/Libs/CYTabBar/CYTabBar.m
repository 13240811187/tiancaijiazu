...
@property (strong , nonatomic) UIView *bottomView;//安全区域空白地 遮挡层 (自己加的)
@end
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
       ...
       self.bottomView.backgroundColor =  [CYTabBarConfig shared].backgroundColor;
...
- (UIView *)bottomView{
    ...
    if (!_bottomView) {
        ...
        _bottomView = [[UIView alloc] initWithFrame:CGRectMake(0,49,[UIScreen mainScreen].bounds.size.width,44)];
            
          }else{
           
              _bottomView = [[UIView alloc] initWithFrame:CGRectMake(0,49,[UIScreen mainScreen].bounds.size.width,0)];
...
- (void)layoutSubviews {
    ...
    CGRect rect = ({
        CGRectMake(0, 0, self.bounds.size.width/count,tabbarHeight);
    });
