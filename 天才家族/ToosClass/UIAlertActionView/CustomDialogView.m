...
-(void)setup
{
    ...
    titleLabel.font = [UIFont boldSystemFontOfSize:17.0f];//字体加粗了
    ...
    _msgLabel.font =  [UIFont fontWithName:@"Arial" size:14.0f];
    ...
    CGFloat empty = _msgLabel.font.pointSize * 0;
    paraStyle01.firstLineHeadIndent = empty;//首行缩进
    paraStyle01.tailIndent = 0.0f;//行尾缩进
    paraStyle01.lineSpacing = 4;//行间距
    
    NSAttributedString *attrText = [[NSAttributedString alloc] initWithString:_message attributes:@{NSParagraphStyleAttributeName:paraStyle01}];
     _msgLabel.attributedText = attrText;
